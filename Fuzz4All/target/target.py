...
INITIAL_WEIGHTS = [
    5,  # BEST_PROMPT_STARCODER,
    5,  # BEST_PROMPT_DEEPSEEK,
    3,  # EVO_PROMPT
    1,  # MACRO_PROMPT
    1,  # MACRO_PROMPT
    1,  # TYPEDEF_PROMPT
...
INITIAL_CHOICES = [
    0, 0, 0, 0, 0,  # BEST_PROMPT_STARCODER,
    1, 1, 1, 1, 1,  # BEST_PROMPT_DEEPSEEK,
    2, 2, 2,  # EVO_PROMPT
    3,  # MACRO_PROMPT
    4,  # MACRO_PROMPT
    5,  # TYPEDEF_PROMPT
...
import os
class Policy():
    """ Q-learning policy. In the fuzz4all scenario, state, action are both
    choices of prompts.
    """
    def __init__(
        self, 
        num_state = len(INITIAL_PROMPTS), 
        ...
        for _ in range(num_state):
            ...
            for k in  range(num_state):
                ...
                vals.append (0.0)
            self.q_values.append(vals)
        self.state = 0   # state
        self.action = 0      # action
        self.time = time.time()
        self.time = str(self.time)[:6]

    def _reward(self, prev) -> float:
        """ given a pair of (compile result, c source code), return the reward. """
        reward = 0
        assert type(prev[0][1]) == type('str')
        for res, fo in prev:
            if res == FResult.SAFE:
                reward += 1
            else:
    ...
    def update_qval(self, prev) -> None:
        """ given the source code, update the q value """
        # increment the counter
        self.counter += 1 
        reward = self._reward(prev)

        # update q value
        prev = self.q_values[self.state][self.action]
        self.q_values[self.state][self.action] = prev * (1.0 -self.lr) + reward * self.lr

        # update current state
        self.state = self.action

    def get_action(self, eplison):
        assert eplison >= 0.0 and eplison <= 1.0
        if self.counter == 0:
            # initially use best prompt
            return 0
        coin = random.random()
        if coin <= eplison:
            # use random policy
            self.action = random.randint(0, self.num_state - 1)
            return self.action
        # follow policy, return best action
        p = 0
        for i in range(1, self.num_state):
            if self.q_values[self.state][i] > self.q_values[self.state][p]:
                p = i
        self.action = p
        return p

def sample_prompt():
    assert len(INITIAL_PROMPTS) > 12
    idx = random.choice(INITIAL_CHOICES)
    return INITIAL_PROMPTS[idx]
...
class Target(object):
    ...
    def auto_prompt(self, **kwargs) -> str:
        ...
        if kwargs["no_input_prompt"]:
            ...
            # best_prompt = BEST_PROMPT_DEEPSEEK  + (
            #     f"{self.prompt_used['separator']}\n{self.prompt_used['begin']}"
            # )
            # best_prompt = EVO_PROMPT 
            best_prompt = MACRO_PROMPT + (
                f"{self.prompt_used['separator']}\n{self.prompt_used['begin']}"
            )
            # best_prompt = EVO_PROMPT
        elif kwargs["hw"]:
    ...
    def update_strategy(self, new_code: str) -> str:
        while 1:
            ...
            # strategy = 0 # only generate new[g]
            # strategy = 1 # only mutate existing[m]
    ...
    def update(self, **kwargs):
        ...
        # may need a scheduler
        # self.initial_prompt = sample_prompt()
        self.initial_prompt = random.choice(INITIAL_PROMPTS)
        # self.policy.update_qval(kwargs["prev"])
        # action = self.policy.get_action(self._eplison())
        # self.initial_prompt = INITIAL_PROMPTS[action]
        new_code = ""
        ...
        if new_code != "" and self.p_strategy != -1:
            # THE CODE LOGIC IS CHANGED HERE!!!
