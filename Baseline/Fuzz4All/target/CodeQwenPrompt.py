...
INASM_PROMPT = """
### Usage of Inline Assembly in C Programming

Inline assembly allows you to embed assembly language instructions directly within your C code. This feature is used when you need to perform low-level hardware manipulations, optimize critical sections of code for performance, or use processor-specific instructions that aren't available in C.

#### Syntax

The syntax for inline assembly varies slightly between different compilers. The most commonly used format is provided by GCC, which uses the `__asm__` or `asm` keyword.

#### Example of Inline Assembly

Here’s a simple example demonstrating the usage of inline assembly to swap two integers using the `XOR` bitwise operation:
...
int main() {
    ...
    // Inline assembly to swap values of a and b using XOR
    __asm__ (
        "xorl %%ebx, %%eax\n\t" // a = a ^ b
        "xorl %%eax, %%ebx\n\t" // b = a ^ b
        "xorl %%ebx, %%eax\n\t" // a = a ^ b
...
1. **Inline Assembly Block**: The `__asm__` keyword is used to start an inline assembly block. Inside the block, assembly instructions are written just as they would be in an assembly program.

2. **XOR Swap Algorithm**: This example uses the XOR bitwise operation to swap the values of `a` and `b` without using a temporary variable.

3. **Constraints**:
   - The `: "=a" (a), "=b" (b)` part specifies that the outputs of the assembly block will be stored in the `a` and `b` variables using the registers `eax` and `ebx`.
   - The `: "a" (a), "b" (b)` part specifies the input values to the assembly block.

4. **Register Usage**: Registers like `eax` and `ebx` are specified by using constraints (`a` and `b`) in the assembly code.

#### Key Points:

- **Performance**: Inline assembly can be used to optimize performance-critical sections of code by utilizing processor-specific instructions.
  
- **Portability**: Inline assembly is not portable; code containing inline assembly is usually tied to a specific architecture and compiler.

- **Low-Level Control**: It allows you to have direct control over the CPU, which is useful for system programming, device drivers, or real-time applications.

This example demonstrates how inline assembly can be embedded in C code to perform operations that might be more efficient or only possible at the assembly level.

#### Instruction
Please write a short c program that uses inline assembly in a complex way. 
...
LABEL_PROMPT = """
### Usage of Label Variables in C Programming

Label variables, also known as "label-as-values" or "computed gotos," are a feature in C that allows you to store the address of a label in a pointer and later use this pointer to jump to that label. This feature is a GNU C extension and is not part of standard C, meaning it is specific to GCC and may not be portable across different compilers.

#### Example of Label Variables

Here’s a simple example demonstrating the usage of label variables:
...
int main() {
    ...
    // Assign the address of a label to the label variable
    label = &&label1;

    // Use a goto statement with the label variable
    goto *label;

label1:
    printf("Jumped to label1\n");

    // Change the label variable to point to another label
    label = &&label2;
    goto *label;
...
1. **Label Declaration**: The `&&label1` syntax is used to obtain the address of the label `label1`, which is then stored in the `label` variable (a pointer to `void`).

2. **Computed Goto**: The `goto *label;` statement uses the label variable to jump to the address stored in it. This allows for dynamic control flow based on the value of the label variable.

3. **Dynamic Jumps**: The program can change the target of the jump by modifying the `label` variable, as shown when it changes from `label1` to `label2`.

#### Key Points:

- **Portability**: Label variables are not part of the standard C language and are specific to GCC. Code that uses label variables may not compile or work as expected on non-GCC compilers.

- **Usage**: This feature is useful in scenarios where dynamic control flow is required, such as in interpreters, state machines, or complex switch-like structures.

- **Caution**: Although powerful, label variables can make code harder to understand and maintain, so they should be used sparingly and with care.

This example illustrates how label variables can be used in C to create dynamic control flow, allowing for jumps to different parts of the code based on runtime conditions.

#### Instruction
Please construct a concise c program that uses label variables in a complex way.
...
int main() {
    ...
    printf("After inner_function: x = %d\n", x);
